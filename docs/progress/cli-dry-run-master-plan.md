...
- 后端 Node（含 viewer 元数据配置）、Library、User、Tag、Auth logout 等写链路已接入 `dryRun`。
...
| 类别 | 范围 | 状态 |
|---|---|---|
| Node | 创建、元数据配置、移动、重命名、删除、恢复、彻删、批量归档、排序等 | 已接入 |
...
节点元数据配置的 dry-run 会执行与真实更新相同的字段、权限、JSON 与标签关系校验，并回滚数据库事务；归档封面和媒体时长预热只在真实提交后触发。
