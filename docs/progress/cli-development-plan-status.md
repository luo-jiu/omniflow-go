...
| M5 | 路径体验层（path -> nodeId） | 待开始 | 在 M4 稳定后推进。 |
...
1. `of fs path resolve`（路径转 nodeId）
2. `of fs cp` / `of fs put`（为自动化与 AI 场景准备）
...
1. 实现 M5 路径体验层（`path -> nodeId`）
2. 增加命令级集成测试（针对 `fs rm/recycle` 的参数和帮助路径）
