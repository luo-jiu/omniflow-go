...
func registerUserRoutes(api *gin.RouterGroup, userHandler *handler.UserHandler) {
	...
	api.GET("/user/me/preferences", userHandler.ListCurrentUserPreferences)
	api.GET("/user/me/preferences/:namespace", userHandler.GetCurrentUserPreference)
	api.PUT("/user/me/preferences/:namespace", userHandler.UpsertCurrentUserPreference)
